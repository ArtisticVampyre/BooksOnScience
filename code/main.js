...
function buildEntries(category) {
    ...
    for (var i = 0; i < tempTable.length; i++) {
        ...
        let drawBox = newBook.querySelector('#rect_draw');
        let drawText = newBook.querySelector('#text_draw');
        if (tempTable[i][1].Rating > 3 && tempTable[i][1].Rating < 4) {
            drawBox.style.fill = DataColor.med;
        } else if (tempTable[i][1].Rating > 3) {
            drawBox.style.fill = DataColor.high;
        } else {
            drawBox.style.fill = DataColor.low;
        }
        drawText.style.fill = '#FFFFFF';
        drawText.innerHTML = tempTable[i][1].Rating
